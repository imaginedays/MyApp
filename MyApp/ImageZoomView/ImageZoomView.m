...
- (instancetype)initWithFrame:(CGRect)frame andWithImage:(UIImageView *)imageview {
    if(self = [super initWithFrame:frame]){
        ...
        [UIView animateWithDuration:0.4 animations:^{
            ...
            [weakSelf.showImgView setFrame:CGRectMake(0, y, width, height)];
            //重要！ 将视图显示出来
            [weakSelf.holderView setAlpha:1];
        ...
        [_holderView addGestureRecognizer:longTapGesture];
        [longTapGesture requireGestureRecognizerToFail:singleTapGestureRecognizer];
...
- (void)singleTap:(UITapGestureRecognizer*)tap {
    __weak typeof(self) weakSelf = self;
    [UIView animateWithDuration:0.4 animations:^{
        [weakSelf.showImgView setFrame:oldframe];
        [weakSelf.holderView setAlpha:0];
    } completion:^(BOOL finished) {
        //完成后操作->将背景视图删掉
